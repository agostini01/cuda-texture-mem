...
__global__ void
device_half_to_float(half *half_array, float *float_array, int width, int height)
{
...
__global__ void
device_float_to_half(float *float_array, half *half_array, int width, int height)
{
    unsigned idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < width * height)
...
int main()
{
    ...
    // Initialize a regular device array and copy host data to device
    half * d_data;
    cudaMalloc(&d_data,width*height*sizeof(half));
